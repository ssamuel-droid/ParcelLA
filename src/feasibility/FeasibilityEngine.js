...
export function generateFeasibilityScenarios(input = {}) {
  ...
  if (profile.group === 'housing' && use !== 'single_family') {
    ...
    if (['commercial', 'industrial'].includes(rule.family)) {
      const ab2011BaseUnits = Math.max(baseUnits, Math.ceil(lotSf / 43560 * 30));
      const ab2011TransitUnits = Math.max(ab2011BaseUnits, Math.ceil(lotSf / 43560 * 80));
      scenarios.push(scenario({ id: 'ab2011', label: 'AB 2011 mixed-income - small site', category: 'State by-right', use, description: 'Mixed-income housing on a qualifying commercial corridor. For a metropolitan site under one acre, the statutory screening density is the greater of local density or 30 units per acre.', program: programFactory(Math.max(baseFar, 2.5), { baseUnits: ab2011BaseUnits, densityMultiplier: 1, heightBonus: 11 }), assumptions, eligibility: 'corridor_and_site_verification_required', requirements: ['Confirm Broadway has a qualifying 70-150 foot commercial-corridor right-of-way at the site.', 'Confirm parcel, environmental, prior-use and tenant-protection eligibility.', 'Meet AB 2011 affordability and labor standards.'], incentives: ['Ministerial approval on qualifying sites', 'Residential use notwithstanding qualifying commercial zoning', 'State Density Bonus may be layered when all requirements are met'], sources, confidence: 'screening' }));
      scenarios.push(scenario({ id: 'ab2011_state_density_bonus', label: 'AB 2011 + 50% State Density Bonus', category: 'Affordable incentive', use, description: 'AB 2011 small-site base density combined with a 50% State Density Bonus screen. The actual bonus depends on the selected affordable unit mix.', program: programFactory(Math.max(baseFar, 3), { baseUnits: ab2011BaseUnits, densityMultiplier: 1.5, heightBonus: 22, parkingRatio: 0.5 }), assumptions, eligibility: 'ab2011_and_affordability_verification_required', requirements: ['First establish AB 2011 eligibility and base density.', 'Select the State Density Bonus affordability mix.', 'Confirm concessions, waivers, replacement housing and labor requirements.'], incentives: ['50% density-bonus screen above the AB 2011 base', 'Potential concessions, waivers and parking relief'], sources, confidence: 'low' }));
      scenarios.push(scenario({ id: 'ab2011_transit', label: 'AB 2011 transit / Very Low VMT', category: 'State by-right', use, description: 'Upper AB 2011 screen for a metropolitan site within one-half mile of a major transit stop or in a Very Low VMT area: the greater of local density or 80 units per acre.', program: programFactory(Math.max(baseFar, 4), { baseUnits: ab2011TransitUnits, densityMultiplier: 1, heightBonus: 22, parkingRatio: 0.5 }), assumptions, eligibility: 'transit_or_vmt_verification_required', requirements: ['Verify the site is within one-half mile of a qualifying major transit stop or in a Very Low VMT area.', 'Confirm all AB 2011 site, affordability, labor and objective-standard requirements.'], incentives: ['80 units/acre statutory screening density in a metropolitan jurisdiction', 'Ministerial review on a qualifying site'], sources, confidence: 'low' }));
      if (inSantaAna) {
        scenarios.push(scenario({ id: 'ab2011_affordable', label: '100% affordable housing - Santa Ana', category: '100% affordable', use, description: 'Santa Ana does not use Los Angeles ED1. This is the comparable AB 2011 100% affordable state-law pathway, screened at 80 units per acre and informed by the nearby 1411 N Broadway approval.', program: programFactory(Math.max(baseFar, 4), { baseUnits: ab2011TransitUnits, densityMultiplier: 1, heightBonus: 22, parkingRatio: 0.3 }), assumptions: affordableAssumptions, eligibility: '100_percent_affordable_and_site_verification_required', requirements: ['Provide 100% lower-income units excluding manager units with the required recorded affordability term.', 'Confirm AB 2011 site eligibility and derive objective standards from the qualifying residential zone required by state law.', 'Verify State Density Bonus concessions/waivers, replacement housing, labor, environmental and Santa Ana requirements.'], incentives: ['No specified maximum density under the AB 2011 100% affordable pathway; objective standards still control', 'State Density Bonus eligibility', 'Nearby 1411 N Broadway precedent: 49 permanent supportive units at about 82 units/acre, with concessions'], sources, confidence: 'low' }));
      }
    }
  ...
  return { use, useLabel: profile.label, zone: rule.zone, zoneRule: rule, lotSf, baseUnits: baseAllowed ? baseUnits : 0, scenarios };
}
