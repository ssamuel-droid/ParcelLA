...
export function generateFeasibilityScenarios(input = {}) {
  ...
  const affordableAssumptions = { ...assumptions, market: { ...(assumptions.market || {}), rentPsfMo: Number(assumptions.affordableRentPsfMo || 2.5), rentBasis: 'Restricted-rent screening blend; replace with the applicable LAHD schedule, bedroom mix and utility allowance.' } };
  const scenarios = [];

  scenarios.push(scenario({ id: 'by_right', label: 'By-right zoning capacity', category: 'By right', use, description: 'Maximum Allowable Residential Density (MARD) from the base zone, shown separately from practical unit sizing and floor-area fit.', program: programFactory(baseFar), assumptions, eligibility: baseAllowed ? 'potentially_eligible' : 'not_indicated', requirements: baseAllowed ? ['Verify general-plan density, specific plans, overlays, setbacks, lot coverage, height district and parking.', 'Confirm unit mix fits within base FAR; MARD is a regulatory unit ceiling, not a plan check.'] : [`${profile.label} is not indicated as a base-zone use in this preliminary screen.`], incentives: [], sources, confidence: input.baseUnits ? 'verified' : rule.recognized ? 'screening' : 'low' }));
