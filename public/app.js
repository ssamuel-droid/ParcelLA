  ...
  <div class="feasibility-panel">
    ...
    <form class="feasibility-form" id="feasibility-form" onsubmit="runFeasibility(event)">
      ...
      <div class="feasibility-advanced" id="feasibility-advanced">
        ...
        <label><span>Rent / SF / month</span><div class="feasibility-money"><b>$</b><input id="fz-rent" type="number" min="0" step="0.05" placeholder="Market"></div></label>
        <label><span>For-sale price / SF</span><div class="feasibility-money"><b>$</b><input id="fz-sale-psf" type="number" min="0" step="5" placeholder="Market"></div></label>
        <label><span>Exit cap rate</span><div class="feasibility-money"><input id="fz-cap" type="number" min="0.1" max="20" step="0.05" placeholder="Market"><b>%</b></div></label>
        <label><span>Vacancy</span><div class="feasibility-money"><input id="fz-vacancy" type="number" min="0" max="100" step="0.5" placeholder="Default"><b>%</b></div></label>
        <label><span>Operating expenses / EGI</span><div class="feasibility-money"><input id="fz-opex" type="number" min="0" max="100" step="0.5" placeholder="Default"><b>%</b></div></label>
        <label><span>Soft costs / hard costs</span><div class="feasibility-money"><input id="fz-soft" type="number" min="0" max="100" step="0.5" placeholder="20"><b>%</b></div></label>
        <label><span>Contingency / hard costs</span><div class="feasibility-money"><input id="fz-contingency" type="number" min="0" max="100" step="0.5" placeholder="5"><b>%</b></div></label>
        <label><span>Loan to cost</span><div class="feasibility-money"><input id="fz-ltc" type="number" min="0" max="100" step="1" placeholder="65"><b>%</b></div></label>
...
function feasibilityRatio(value) {
  const n = Number(value);
...
function renderFeasibilityResult() {
  ...
  const landBasis = data.landBasis || {};
  const landBasisLabel = landBasis.imputed ? 'Imputed land basis' : 'Acquisition basis';
  ...
  const incomeModel = ['housing', 'commercial', 'hospitality'].includes(u.assumptions?.group);
  ...
  body.innerHTML = `
    <div class="fz-summary">
      ...
      <div class="fz-fact"><span>${landBasisLabel}</span><strong>${feasibilityValue(landBasis.value)}</strong><small style="color:#7d8999;font-size:8px">${escapeText(landBasis.formula || landBasis.source || 'Enter acquisition price')}</small></div>
    ...
    <div class="fz-layout">
      ...
      ${selected ? `<article class="fz-detail">
        ...
        <div class="fz-kpis">
          ...
          <div class="fz-kpi"><span>Equity required</span><b>${feasibilityValue(u.equity)}</b></div>
          <div class="fz-kpi"><span>Net sale proceeds</span><b>${feasibilityValue(u.netSaleProceeds)}</b></div>
          <div class="fz-kpi"><span>Profit / cost</span><b>${feasibilityPct(u.marginOnCost)}</b></div>
        </div>
        <div class="fz-columns">
          <section class="fz-section"><h3>Concept program</h3><table class="fz-table"><tr><td>Density basis</td><td>${escapeText(p.densityMode === 'floor_area' ? 'Limited by floor area' : p.densityMode === 'verified_project' ? 'Verified City project' : 'Zoning MARD')}</td></tr><tr><td>Base MARD</td><td>${fmtN(p.baseUnits || data.baseUnits || 0)} units</td></tr><tr><td>Gross building area</td><td>${fmtN(p.grossSf)} SF</td></tr>${p.grossSfCapacity > p.grossSf ? `<tr><td>Maximum FAR envelope</td><td>${fmtN(p.grossSfCapacity)} SF</td></tr>` : ''}<tr><td>Modeled / maximum FAR</td><td>${Number.isFinite(Number(p.far)) ? Number(p.far).toFixed(2) + ':1' : 'n/a'}${Number.isFinite(Number(p.maximumFar)) && Math.abs(Number(p.maximumFar) - Number(p.far)) > 0.01 ? ` / ${Number(p.maximumFar).toFixed(2)}:1` : ''}</td></tr><tr><td>Net / rentable area</td><td>${fmtN(p.netSf)} SF</td></tr><tr><td>Residential units / rooms</td><td>${fmtN(p.units || 0)}</td></tr><tr><td>Average net unit SF</td><td>${p.avgUnitSf ? fmtN(p.avgUnitSf) + ' SF' : 'n/a'}</td></tr><tr><td>Commercial area</td><td>${fmtN(p.commercialSf || 0)} SF</td></tr><tr><td>Estimated stories</td><td>${fmtN(p.stories)}</td></tr><tr><td>Screened height</td><td>${fmtN(p.heightFt)} ft</td></tr><tr><td>Parking screen</td><td>${fmtN(p.parkingSpaces)} spaces</td></tr></table></section>
          <section class="fz-section"><h3>Sources & uses</h3><table class="fz-table"><tr><td>${landBasis.imputed ? 'Imputed land basis' : 'Land / acquisition'}${landBasis.formula ? `<small style="display:block;color:#7d8999">${escapeText(landBasis.formula)}</small>` : ''}</td><td>${feasibilityValue(u.landCost)}</td></tr><tr><td>Hard costs (${feasibilityValue(u.hardCostPsf)}/SF)</td><td>${feasibilityValue(u.hardCosts)}</td></tr><tr><td>Soft costs</td><td>${feasibilityValue(u.softCosts)}</td></tr><tr><td>Contingency</td><td>${feasibilityValue(u.contingency)}</td></tr><tr><td>Construction financing carry</td><td>${feasibilityValue(u.financing)}</td></tr><tr><td><b>Total development cost</b></td><td><b>${feasibilityValue(u.totalCost)}</b></td></tr><tr><td>Construction loan</td><td>${feasibilityValue(u.loanAmount)}</td></tr><tr><td><b>Equity required</b></td><td><b>${feasibilityValue(u.equity)}</b></td></tr><tr><td>Cost / gross SF</td><td>${feasibilityValue(u.costPerSf)}</td></tr><tr><td>Cost / unit</td><td>${p.units ? feasibilityValue(u.costPerUnit) : 'n/a'}</td></tr></table></section>
        </div>
        <div class="fz-columns">
          ${incomeModel ? `<section class="fz-section"><h3>Stabilized operating statement</h3><table class="fz-table"><tr><td>Gross potential revenue</td><td>${feasibilityValue(u.grossRevenue)}</td></tr><tr><td>Vacancy / credit loss</td><td>(${feasibilityValue(u.vacancyLoss)})</td></tr><tr><td>Effective gross income</td><td>${feasibilityValue(u.effectiveGrossIncome)}</td></tr><tr><td>Operating expenses</td><td>(${feasibilityValue(u.operatingExpenses)})</td></tr><tr><td><b>Net operating income</b></td><td><b>${feasibilityValue(u.noi)}</b></td></tr><tr><td>Yield on cost</td><td>${feasibilityPct(u.capOnCost)}</td></tr></table></section>` : `<section class="fz-section"><h3>For-sale revenue</h3><table class="fz-table"><tr><td>Gross sellout</td><td>${feasibilityValue(u.grossRevenue)}</td></tr><tr><td>Sale costs</td><td>(${feasibilityValue(u.dispositionCosts)})</td></tr><tr><td><b>Net sellout proceeds</b></td><td><b>${feasibilityValue(u.netSaleProceeds)}</b></td></tr><tr><td>Sale price / net SF</td><td>${feasibilityValue(u.assumptions?.salePsf)}</td></tr></table></section>`}
          <section class="fz-section"><h3>Capitalization & returns</h3><table class="fz-table"><tr><td>Gross exit value</td><td>${feasibilityValue(u.exitValue)}</td></tr><tr><td>Disposition costs</td><td>(${feasibilityValue(u.dispositionCosts)})</td></tr><tr><td>Net sale proceeds</td><td>${feasibilityValue(u.netSaleProceeds)}</td></tr><tr><td><b>Development profit</b></td><td><b>${feasibilityValue(u.profit)}</b></td></tr><tr><td>Profit / total cost</td><td>${feasibilityPct(u.marginOnCost)}</td></tr><tr><td>Unlevered IRR</td><td>${feasibilityPct(u.unleveredIrr)}</td></tr><tr><td>Levered IRR</td><td>${feasibilityPct(u.leveredIrr)}</td></tr><tr><td>Return on equity</td><td>${feasibilityPct(u.returnOnEquity)}</td></tr><tr><td>Equity multiple</td><td>${feasibilityRatio(u.equityMultiple)}</td></tr>${incomeModel ? `<tr><td>Annual debt service</td><td>${feasibilityValue(u.annualDebtService)}</td></tr><tr><td>Cash flow before tax</td><td>${feasibilityValue(u.cashFlowBeforeTax)}</td></tr><tr><td>DSCR</td><td>${feasibilityRatio(u.dscr)}</td></tr><tr><td>Debt yield</td><td>${feasibilityPct(u.debtYield)}</td></tr><tr><td>Loan / value</td><td>${feasibilityPct(u.ltv)}</td></tr>` : ''}</table></section>
        </div>
        <section class="fz-section"><h3>Underwriting assumptions</h3><table class="fz-table fz-assumptions"><tr><td>Hard cost</td><td>${feasibilityValue(u.hardCostPsf)}/SF</td><td>Soft costs</td><td>${feasibilityPct(u.assumptions?.softCostPct)}</td></tr><tr><td>Contingency</td><td>${feasibilityPct(u.assumptions?.contingencyPct)}</td><td>Loan to cost</td><td>${feasibilityPct(u.assumptions?.ltc)}</td></tr><tr><td>Interest / amortization</td><td>${feasibilityPct(u.assumptions?.interestRate)} / ${fmtN(u.assumptions?.amortizationYears)} yr</td><td>Construction period</td><td>${fmtN(u.assumptions?.constructionMonths)} months</td></tr>${incomeModel ? `<tr><td>Rent / SF / month</td><td>${feasibilityValue(u.assumptions?.rentPsfMo)}</td><td>Vacancy</td><td>${feasibilityPct(u.assumptions?.vacancy)}</td></tr><tr><td>Operating expenses / EGI</td><td>${feasibilityPct(u.assumptions?.opex)}</td><td>Exit cap rate</td><td>${feasibilityPct(u.assumptions?.capRate)}</td></tr>` : `<tr><td>Sale price / net SF</td><td>${feasibilityValue(u.assumptions?.salePsf)}</td><td>Sale costs</td><td>${feasibilityPct(u.assumptions?.exitCostPct)}</td></tr>`}</table></section>
        <div class="fz-columns">
...
async function runFeasibility(event) {
  ...
  const request = {
    ...
    rentPsfMo: g('fz-rent')?.value, salePsf: g('fz-sale-psf')?.value,
    capRate: g('fz-cap')?.value, vacancyPct: g('fz-vacancy')?.value,
    opexPct: g('fz-opex')?.value, softCostPct: g('fz-soft')?.value,
    contingencyPct: g('fz-contingency')?.value, ltc: g('fz-ltc')?.value,
    constructionMonths: g('fz-months')?.value, amortizationYears: g('fz-amortization')?.value,
    exitCostPct: g('fz-exit-cost')?.value,
    landPerDoor: baseUserMetrics().imputedLandPerDoorMarket,
    landPerLotSf: baseUserMetrics().imputedHouseLandPerLotSf,
  };
...
function imputedLandFromDoorSetting(s) {
  if (!canUseDoorLandBasis(s)) return 0;
