...
router.post('/underwriting', requireAuth, requireActiveAccess, async (req, res, next) => {
  try {
    ...
    const planningDocumentCandidates = [
      ...planningCases.flatMap(planningCase =>
        tableRows(planningCase.documents || [], 250).map(document => ({ caseNumber: planningCase.caseNumber, ...document }))
      ),
      ...tableRows(planning.documents || [], 250),
    ...
    const planningDocuments = [...new Map(planningDocumentCandidates.map(document => [document.url, document])).values()];
    if (planningDocuments.length) {
      planningDocuments.forEach(document => writeRow(planningWs, [
        ...
        document.url ? { text: 'Open PDF', hyperlink: text(document.url) } : '',
      ]));
    } else {
      writeRow(planningWs, ['No verified planning PDF is available for the matched cases yet.']);
    }

    let rentAnnualRef = '';
    if (isHouse) {
      const houseInputWs = wb.addWorksheet('House Sale Inputs');
    ...
    let incomeWs;
    if (isHouse) {
      incomeWs = wb.addWorksheet('Project Economics');
      setupSheet(incomeWs, [34, 20, 58]);
      writeRow(incomeWs, ['SFR Project Economics', siteName], 'title');
      writeRow(incomeWs, ['Metric', 'Value', 'Formula / notes'], 'header');
    ...
    if (isHouse) {
      ...
      valRow('leveredIrr', 'Levered IRR', `IFERROR(XIRR(${rangeRef('Cash Flow', 5, 2, 5, 3)},${rangeRef('Cash Flow', 2, 2, 2, 3)}),0)`, pctPoints(valuation.leveragedIRR), FMT.pct, 'Annualized construction-period equity return from Cash Flow tab.');
    } else {
    ...
    setupSheet(scenarioWs, [24, 14, 12, 12, 16, 16, 16, 16, 16, 16, 14, 42]);
    writeRow(scenarioWs, ['Construction Scenarios', siteName], 'title');
    writeRow(scenarioWs, isHouse
      ? ['Plan', 'Hard / SF', 'Soft %', 'Months', 'Resale / SF', 'Total cost', 'Completed value', 'Net profit', 'Total cost / SF', 'Return on cost', 'Gross margin', 'Notes']
      : ['Plan', 'Hard / SF', 'Soft %', 'Months', 'Rent impact', 'Total cost', 'NOI', 'Exit value', 'Net profit', 'Cost / unit', 'Cap on cost', 'Notes'], 'header');
    ...
    writeRow(salesWs, isHouse
      ? ['Address', 'Distance', 'Sale date', 'Sale price', 'Houses', 'Building SF', 'Year built', 'Property type', '$ / house', '$ / SF', 'Weight', 'Source / notes']
      : ['Address', 'Distance', 'Sale date', 'Sale price', 'Units', 'Avg SF', 'Year built', 'Cap rate', '$ / Unit', '$ / SF', 'Weight', 'Source / notes'], 'header');
    salesComps.forEach(c => {
      ...
      writeRow(salesWs, [
        ...
        blankUnlessPositive(isHouse ? (c.buildingSf || c.usableBuildingSf || c.avgUnitSf) : c.avgUnitSf, FMT.whole),
        blankUnlessPositive(c.yearBuilt, FMT.whole),
        isHouse ? text(c.projectType || 'Single family') : blankPct(cap, FMT.pct2),
        formula(`IFERROR(D${r}/E${r},0)`, money(c.pricePerUnit || c.usablePricePerUnit), FMT.money),
        formula(isHouse ? `IFERROR(D${r}/F${r},0)` : `IFERROR(D${r}/(E${r}*F${r}),0)`, money(c.pricePerSf || c.usablePricePerSf), FMT.money),
    ...
    if (isHouse) {
      writeRow(appraisalWs, ['Subject completed home SF', 'n/a', formula(`${V.homeSf}`, totalSfValue, FMT.whole), text(site.buildingSfSource || 'Permit/source data')]);
    ...
    if (isHouse) {
      ...
      writeRow(summaryWs, ['Comp-derived resale value / SF', formula(`${V.resalePsf}`, money(appraisal.weightedPsf), FMT.money), text(appraisal.valuationSource || assumptions.resalePricePerSfSource)]);
    } else {
    ...
    if (isHouse) {
      ...
      writeRow(summaryWs, ['Return on cost', formula(`${V.returnOnCost}`, pct(valuation.returnOnCost), FMT.pct), 'Net profit / total project cost']);
      writeRow(summaryWs, ['Valuation method', 'Sales comparison - price per building SF', 'Recent local completed-home sales; fallback is explicitly labeled.']);
    } else {
